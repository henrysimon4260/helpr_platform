...
import { PlaceSuggestion } from '../../../components/services/LocationAutocompleteInput';
...
export function useLocationManagement({ showModal, mapRef, resetPriceState }: LocationManagementProps) {
  ...
  // Animate map to location when it changes
  useEffect(() => {
...
export function useVoiceInput({ setDescription, resetPriceState, showModal }: VoiceInputProps) {
    ...
    try {
      ...
      if (transcript) {
        ...
        setDescription(prev => {
          ...
          return trimmedPrev.length > 0 ? `${trimmedPrev} ${transcript}`.trim() : transcript;
        });
      }
    } finally {
  ...
  return { isRecording, isTranscribing, voicePulseValue, handleVoiceModePress };
}
...
interface CleaningAnalysisProps {
  ...
  fetchPriceEstimate: (
    desc: string,
    options?: { start?: SelectedLocation | null; end?: SelectedLocation | null },
  ) => Promise<void | { needsClarification: true }>;
  checkForPropertySize: (text: string) => boolean;
  checkForCleaningType: (text: string) => boolean;
  checkIfDescriptionAlreadyEnhanced: (text: string) => boolean;
  isPriceLoading: boolean;
  isTranscribing: boolean;
  showModal: (config: { title: string; message: string }) => void;
}

export function useCleaningAnalysis({
  description,
  location,
  locationQuery,
  setDescription,
  ...
  const resetCleaningAnalysisFlow = useCallback(() => {
  ...
  const analyzecleaningDescription = useCallback(
    (text: string) => {
      const lowerText = text.toLowerCase();
      const spelledOutBedroomPattern =
        /\b(one|two|three|four|five|six|seven|eight|nine|ten|single|double|triple)\s*(?:-|\s)?\s*(bedroom|bed|br|room|apt|apartment)s?\b/;
  ...
  const handleAnalysisModalSubmit = useCallback(() => {
    resetCleaningAnalysisFlow();
    handleDescriptionSubmit();
  }, [handleDescriptionSubmit, resetCleaningAnalysisFlow]);
  ...
  }, [currentQuestionStep, resetCleaningAnalysisFlow]);
...
interface ServiceSubmissionProps {
  ...
  collectFormState: () => CleaningFormState;
  restoreFormState: (state: CleaningFormState) => void;
}
...
export function useServiceSubmission({
  ...
  const { setReturnTo, getReturnTo, clearReturnTo } = useAuth();
  ...
  useEffect(() => {
    ...
    if (!returnTo || returnTo.path !== CLEANING_RETURN_PATH || !returnTo.data) return;
    ...
    clearReturnTo();
    if (payload.action === 'schedule-cleaning') setPendingResumeAction('schedule-cleaning');
  }, [user, getReturnTo, clearReturnTo, restoreFormState]);

  // Resume scheduling after sign-in
  useEffect(() => {
    if (!user || pendingResumeAction !== 'schedule-cleaning' || isSubmitting) return;
