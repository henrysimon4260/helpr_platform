...
export default function Account() {
  ...
  const [paymentMethods, setPaymentMethods] = useState<SavedPaymentMethodSummary[]>([]);
  ...
  const fetchPaymentMethods = async () => {
    try {
      ...
      // Set active payment method to the default one
  ...
  const addPaymentMethod = async (isDefault: boolean = false) => {
    if (!cardComplete) {
      presentModal('Error', 'Please enter valid card details');
      return;
    }
    ...
    try {
      ...
      const { data: { user } } = await supabase.auth.getUser();
      if (!user?.id) {
        presentModal('Error', 'Please sign in to add a payment method');
        return;
      }

      const result = await createPaymentMethod({ paymentMethodType: 'Card' });
      if (result.error || !result.paymentMethod) {
        presentModal('Error', result.error?.message || 'Failed to create payment method');
        return;
      }
      ...
      const brand = result.paymentMethod.Card?.brand || 'Card';
      const normalizedBrand = brandMap[brand.toLowerCase()] || brand.charAt(0).toUpperCase() + brand.slice(1);

      const savedMethod = await savePaymentMethod(
        user.id,
        result.paymentMethod.id,
        normalizedBrand,
        cardDetailsSnapshot?.last4 ?? result.paymentMethod.Card?.last4 ?? '0000',
        cardDetailsSnapshot?.expiryMonth ?? result.paymentMethod.Card?.expMonth ?? 12,
        cardDetailsSnapshot?.expiryYear ?? result.paymentMethod.Card?.expYear ?? new Date().getFullYear() + 3,
        isDefault
      );
      ...
      if (savedMethod) {
        if (isDefault) {
          ...
          setPaymentMethods(prev => [...prev.map(m => ({ ...m, isDefault: false })), { ...savedMethod, isDefault: true }]);
        } else {
        ...
        presentModal('Success', 'Payment method added successfully!');
      }
    } catch (error) {
      console.error('Error saving payment method:', error);
      presentModal('Error', 'Failed to save payment method');
    } finally {
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <PaymentMethodModal
        ...
        showAddForm={showAddPayment}
        setShowAddForm={setShowAddPayment}
